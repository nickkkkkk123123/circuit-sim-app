...
describe('运动学沙盒（物理引擎）', () => {
  ...
  it('恢复系数 e=0.5：正碰后分离速度为接近速度的一半', () => {
    const balls = [makeBall(1, 5, 10, 0.5, 2, 0, 0.5), makeBall(2, 5.9, 10, 0.5, 0, 0, 0.5)]
    ballCollisions(balls)
  ...
  it('右墙反弹：vx 反向 × e，位置钳位不出墙', () => {
    const balls = [makeBall(1, 39.8, 10, 0.5, 5, 0, 0.8)]
  ...
  it('分离中的两球（vn≥0）不施加冲量', () => {
    ...
    ballCollisions(balls) // 重叠但正在分离：只做位置修正，不改速度
  ...
  it('静态体：圆弧把球导向弧面滑动，不被穿透', () => {
    ...
    const balls = [makeBall(1, 14.2, 9, 0.4, 3, 0, 0.6)]
    stepSandbox(balls, statics, P({ g: 10 }), 2, 240)
